...
function mcl_core.generate_v6_oak_tree(p)
	...
	for dy = 0, 4 do
		pos.y = p.y + dy
		if minetest.get_node(pos).name == "air" then minetest.add_node(pos, trunk) end
	end
	...
	return function(pos)
		...
		if stage >= 3 then
			...
			if two_by_two then
				...
				-- clockwise from +x
				local p2 = vector_offset(pos,  1, 0,  0)
				local p3 = vector_offset(pos,  1, 0, -1)
				local p4 = vector_offset(pos,  0, 0, -1)
				local p5 = vector_offset(pos, -1, 0, -1)
				local p6 = vector_offset(pos, -1, 0,  0)
				local p7 = vector_offset(pos, -1, 0,  1)
				local p8 = vector_offset(pos,  0, 0,  1)
				local p9 = vector_offset(pos,  1, 0,  1)
				...
				if s2 and s3 and s4 and check_tree_growth(pos, tree_id, { two_by_two = true }) then
					...
					return
				elseif s4 and s5 and s6 and check_tree_growth(p6, tree_id, { two_by_two = true }) then
					minetest.remove_node(pos)
					minetest.remove_node(p4)
				...
				elseif s6 and s7 and s8 and check_tree_growth(p7, tree_id, { two_by_two = true }) then
					...
					return
				elseif s8 and s9 and s2 and check_tree_growth(p8, tree_id, { two_by_two = true }) then
					minetest.remove_node(pos)
					minetest.remove_node(p8)
